...
{
    public class CircleMaster : COM.DALInterface , IDisposable
    {
        DAL.CRUDOperation objDAL = new DAL.CRUDOperation();
        DAL.CircleMaster clsDAL = new DAL.CircleMaster();
        public ENT.CircleMaster Entity = new ENT.CircleMaster();
        List<ENT.CircleMaster> lstEntity;
        ...
        private List<string> ValidationEntry(object obj)
        {
            ...
            if (string.IsNullOrWhiteSpace(Entity.circle_name)) { strvalidationResult.Add("Circle Name is Required!"); }
            if (string.IsNullOrWhiteSpace(Entity.circle_code)) { strvalidationResult.Add("Circle Code is Required!"); }
        ...
        public bool Insert(object obj)
        {
            ...
            try
            {
                ...
                {
                    COM.HelperMethod.SetValueInObject(obj, Guid.NewGuid(), "circleid");
        ...
        public List<ENT.CircleMaster> GetAll(string search)
        {
            lstEntity = new List<ENT.CircleMaster>();
            lstEntity = clsDAL.GetList(search);
            return lstEntity;
        }
        public List<ENT.CircleMaster> GetAllActive(string country_code)
        {
            lstEntity = new List<ENT.CircleMaster>();
            lstEntity = clsDAL.GetListActived(country_code);
            return lstEntity;
        }
        public List<ENT.CircleMaster> CheckDuplicateCombination(List<Guid> ParentID, string strName, string strName2)
        {
            lstEntity = new List<ENT.CircleMaster>();
        ...
        public bool UpdateStatus(Guid PrimarKey, COM.MyEnumration.MyStatus Status)
        {
            ...
            try
            {
                ...
                Dictionary<string, bool> dctFields = new Dictionary<string, bool>();
                dctFields.Add(COM.HelperMethod.PropertyName<ENT.CircleMaster>(x => x.circleid), true);
                dctFields.Add(COM.HelperMethod.PropertyName<ENT.CircleMaster>(x => x.Status), false);
                Entity.circleid = PrimarKey;
